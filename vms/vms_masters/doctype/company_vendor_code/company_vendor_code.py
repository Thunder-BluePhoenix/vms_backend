...
class CompanyVendorCode(Document):
	...
	def on_update(self):
		"""Handle document update with proper error handling and logging"""
		try:
			vend = frappe.get_doc("Vendor Master", self.vendor_ref_no)
			found = False
			
			# Update or create multiple company data entry
			...
			if vend.vendor_onb_records:
				try:
					...
					frappe.log_error(f"Error getting vendor onboarding data: {str(e)}", "Company Vendor Code")

			# Collect all vendor code data for PDF - WITH PROPER ERROR HANDLING
			vendor_code_data = self.collect_vendor_code_data_safe(vend)

			# Handle user creation and email sending
			if vend.via_data_import == 0:
				self.handle_user_creation_and_email(vend, vendor_code_data, cc)
				
		except Exception as e:
			frappe.log_error(f"Error in Company Vendor Code on_update: {str(e)}", "Company Vendor Code Error")
			# Don't re-raise to prevent blocking the save operation

	def collect_vendor_code_data_safe(self, vendor_doc):
		"""Collect all vendor code data with proper error handling - FIXED VERSION"""
		all_vendor_data = []
			...
			for company_data_row in vendor_doc.multiple_company_data:
				if hasattr(company_data_row, 'company_vendor_code') and company_data_row.company_vendor_code:
					try:
						...
						frappe.log_error(f"Error fetching Company Vendor Code {company_data_row.company_vendor_code}: {str(e)}", "Company Vendor Code")
						continue
						
		except Exception as e:
			frappe.log_error(f"Error in collect_vendor_code_data_safe: {str(e)}", "Company Vendor Code")
		
		# FIXED: Use proper logging instead of print() which causes BrokenPipeError
		frappe.logger().info(f"Collected vendor code data: {len(all_vendor_data)} records")
		
		return all_vendor_data

	def handle_user_creation_and_email(self, vend, vendor_code_data, cc):
		"""Handle user creation and email sending with proper error handling"""
		try:
			...
			frappe.log_error(f"Error in handle_user_creation_and_email: {str(e)}", "Company Vendor Code")

	def collect_vendor_code_data(self, vendor_doc):
		"""Legacy method - redirects to safe version"""
		return self.collect_vendor_code_data_safe(vendor_doc)
	...
	def send_vendor_email_with_pdf(self, email, username, password, vendor_name, vendor_code_data, is_new_user=True, cc=None):
		...
		try:
			...
			if is_new_user and password:
				...
				subject = f"Vendor Codes Update - {vendor_name}"
				message = f"""
				<p>Dear {vendor_name},</p>
				<p>Your vendor codes have been updated. Please find the latest information in the attached PDF.</p>
				<p>Thank you!</p>
				"""
			...
			frappe.log_error(f"Error sending vendor email: {str(e)}", "Company Vendor Code Email")
	...
	def generate_vendor_code_pdf(self, vendor_name, vendor_code_data):
		"""Generate PDF with vendor code information"""
		try:
			html_content = f"""
			<h2>Vendor Code Information</h2>
			...
			for data in vendor_code_data:
				html_content += f"""
				<tr>
					<td>{data.get('company_name', 'N/A')}</td>
					<td>{data.get('state', 'N/A')}</td>
					<td>{data.get('gst_no', 'N/A')}</td>
					<td>{data.get('vendor_code', 'N/A')}</td>
				</tr>
				"""
			...
			frappe.log_error(f"Error generating PDF: {str(e)}", "Company Vendor Code PDF")
			return None
