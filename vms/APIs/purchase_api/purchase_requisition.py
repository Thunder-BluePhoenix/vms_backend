		...
		for row in sorted(doc.purchase_requisition_form_table, key=lambda x: x.idx):
			...
			if head_id not in grouped_data:
				grouped_data[head_id] = {
					...
					"store_location_head_desc": frappe.db.get_value("Storage Location Master", row.store_location_head, ["name", "storage_location", "storage_location_name", "description"], as_dict=True) or {},
					"item_category_head": row.item_category_head,
					"material_group_head": row.material_group_head,
					"material_group_head_desc": frappe.db.get_value("Material Group Master", row.material_group_head, ["name", "material_group_name", "material_group_description"], as_dict=True) or {},
					...
					"purchase_group_head_desc": frappe.db.get_value("Purchase Group Master", row.purchase_group_head, ["name", "purchase_group_code", "purchase_group_name", "description"], as_dict=True) or {},
					...
					"plant_head_desc": frappe.db.get_value("Plant Master", row.plant_head, ["name", "plant_code", "plant_name", "description"], as_dict=True) or {},
					...
					"valuation_area_head_desc": frappe.db.get_value("Valuation Class", row.valuation_area_head, ["name", "valuation_class_code", "valuation_class_name"], as_dict=True) or {},
