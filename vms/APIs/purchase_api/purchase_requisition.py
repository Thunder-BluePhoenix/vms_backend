		...
		for row in sorted(doc.purchase_requisition_form_table, key=lambda x: x.idx):
			...
			if head_id not in grouped_data:
				grouped_data[head_id] = {
					...
					"store_location_head_desc": frappe.db.get_value("Storage Location Master", row.store_location_head, "storage_location_name"),
					"item_category_head": row.item_category_head,
					"material_group_head": row.material_group_head,
					"material_group_head_desc": frappe.db.get_value("Material Group Master", row.material_group_head, "material_group_description"),
					...
					"plant_head_desc": frappe.db.get_value("Plant Master", row.plant_head, "plant_name"),
					...
					"valuation_area_head_desc": frappe.db.get_value("Valuation Class", row.valuation_area_head, "valuation_class_code")+"-"+frappe.db.get_value("Valuation Class", row.valuation_area_head, "valuation_class_name"),
