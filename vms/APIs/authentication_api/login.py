...
def login(data):
    try:
        ...
        # Generate API credentials after successful authentication
        api_credentials = generate_api_keys(frappe.session.user)

        # Build user response
        user_response = build_user_response(frappe.session.user, api_credentials)

        # Set response type and success message
        frappe.local.response["type"] = "json"
        frappe.local.response["message"] = {
            "success": True, 
            "message": _("Authentication successful")
        ...
        frappe.logger().error(f"Authentication error for user {usr}: {str(e)}")
        frappe.local.response["message"] = {
            "success": False,
            "message": _("Invalid username or password")
        }
        ...
        frappe.logger().error(f"Validation error during login: {str(e)}")
        frappe.local.response["message"] = {
            "success": False,
            "message": str(e)
        }
        ...
        frappe.logger().error(f"Login error for user {usr}: {str(e)}")
        frappe.local.response["message"] = {
            "success": False,
            "message": _("An error occurred during authentication")
        }
...
def build_user_response(user, api_credentials):
    ...
    try:
        ...
        frappe.logger().error(f"Error building user response: {str(e)}")
        raise
...
# Separate logout function to clear cookies
@frappe.whitelist(allow_guest=True)
def logout():
    """Logout function to clear authentication cookies"""
    try:
        # Clear session
